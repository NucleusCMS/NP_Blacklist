	...
	if (file_exists(__WEBLOG_ROOT.__EXT.'/settings/personal_blacklist.pbl')) {
		...
		while (!feof($handle)) {
			...
			if (strlen($key) > 0) {
				...
				echo "<td>";
				echo "<a href=\"".hsc($manager->addTicketToUrl(serverVar('PHP_SELF')."?action=deleteexpression&line=".$line))."\">".NP_BLACKLIST_delete."</a>";
				echo "</td>";
				echo "</tr>\n";
			}
		}
	...
	if (isset ($_POST["expression"])) {
		...
		if ($expression != "") {
			...
			if ($enable_regex && (!pbl_checkregexp($expression))) {
				return "<div class=\"pblmessage\">".NP_BLACKLIST_containdError.": <b>".hsc($expression)."</b></div>\n";
			} else {
				$existTest = pbl_checkforspam($expression);
				if (strlen($existTest) > 0) {
					return "<div class=\"pblmessage\"><b>".hsc($expression)."</b> ".NP_BLACKLIST_alreadyMatched.": <b>$existTest</b></div>\n";
				} else {
					pbl_addexpression($expression, $comment);
					return "<div class=\"pblmessage\">".NP_BLACKLIST_newEntryAdded.": <b>".hsc($expression)."</b></div>";
				}
			}
		} else {
			return "<div class=\"pblmessage\">".NP_BLACKLIST_emptyExpression.": <b>".hsc($expression)."</b></div>";
		}
	}
	return '';
}
...
function pbl_showIp($type = 'block') {
	...
	while ($ip = fgets($fp, 255)) {
		...
		echo "<a href=\"".hsc($manager->addTicketToUrl(serverVar('PHP_SELF').'?action=deleteip'.$type.'&line='.$line))."\">".NP_BLACKLIST_delete."</a>";
		echo "</td></tr>";
	}
}
...
function pbl_addIp($type = 'block') {
	if (isset ($_POST["ipaddress"])) {
		...
		return "<div class=\"pblmessage\">".NP_BLACKLIST_newEntryAdded.": <b>".hsc(postVar("ipaddress"))."</b></div>";
	}
	return '';
}
...
function pbl_test() {
	...
	if (isset ($_POST["expression"])) {
		if (postVar("expression") != "") {
			$pblmessage = NP_BLACKLIST_yourExpression.": <br /><pre>".hsc(postVar("expression")).'</pre>';
